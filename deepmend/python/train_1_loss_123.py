...
def main_function(experiment_directory, continue_from, batch_split):
    ...
    use_break_loss = specs.get("BreakLoss", True)  ###break loss on False; break loss= loss for break set
    # >>> end update
    ...
    tb_logger = torch.utils.tensorboard.writer.SummaryWriter(log_dir="tensorboard/train_original_{}".format(latent_size))
    ...
    for epoch in tqdm.tqdm(
        ...
        for data, inds in sdf_loader:
            ...
            for i in range(batch_split):
                ...
                chunk_loss = (
                    loss_bce_with_logits(c_x, c_gt)   ##### no loss of complete shape
                    + loss_bce(b_x, b_gt)
                    + loss_bce(r_x, r_gt)
                ) / num_sdf_samples
                
                tb_logger.add_scalar("Fractured loss", loss_bce(b_x, b_gt).item(), epoch)
                tb_logger.add_scalar("Reconstruction loss", loss_bce(r_x, r_gt).item(), epoch)
                ...
                if do_code_regularization:
                    ...
                    l1_size_loss = torch.sum(torch.norm(batch_vecs, dim=1)**2) + torch.sum(
                        torch.norm(tool_batch_vecs, dim=1)**2
                    )
                    # >>> end update
