...
def partial_sample_pointnet(
    ...
    core.vis.ply_samples(
        "classifier.ply",
        (vertices, mask.astype(float))
    )
...
class SamplesDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        logging.info("Using return tool:    {}".format(self._return_tool))
        ...
        # Tried something else here but I'm not going to use it
        # if "occ_values" in self._raw_data:
        #     logging.info("Found 'occ_values' attribute in data dict, hstacking points and occ values")
        #     for idx in range(len(self._data)):
        #         self._data[idx] = np.hstack((
        #             self._data[idx],
        #             self._raw_data["occ_values"][idx].astype(float),
        #         ))
    ...
    def __getitem__(self, idx):
        ...
        if self._learned_breaks:
            ...
            sdf_data = [
                ...
                torch.from_numpy(values[:, 1]).unsqueeze(1),  # Broken
                torch.from_numpy(values[:, 2]).unsqueeze(1),  # Restoration
            ]
            if self._return_tool:
                sdf_data.append(
                    torch.from_numpy(values[:, 3]).unsqueeze(1),  # Tool
                )
