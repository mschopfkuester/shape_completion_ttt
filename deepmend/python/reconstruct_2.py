...
def reconstruct(
    ...
    loss_version=None,
    path_latent=None,
    path_tool_latent=None
):
    tb_logger = torch.utils.tensorboard.writer.SummaryWriter(log_dir="object no. "+str(4))
    #print(decoder,num_iterations,latent_size,break_latent_size,test_sdf,stat)
    def adjust_learning_rate(
        ...
        for param_group in optimizer.param_groups:
            param_group["lr"] = lr
    def adjust_learning_rate_new(optimizer, lr_warmup,warmup_length,lr_end,it_step, num_step):
    ...
    decreased_by = 10  ###10
    adjust_lr_every =int(num_iterations / 2)#500

    if type(stat) == type(0.1):
        #latent = torch.ones(1, latent_size).normal_(mean=0, std=stat).cuda()
        #break_latent = torch.ones(1, break_latent_size).normal_(mean=0, std=stat).cuda()
        #print(latent.size(),break_latent.size())
        latent=torch.ones(1,1000).cuda()
        break_latent=torch.ones(1,1000).cuda()
        #print(path_latent,path_tool_latent)
        if path_latent is not None and path_tool_latent is not None:
            #print('load latent code')
            latent[0]=torch.load(path_latent)['latent_codes']['weight']['weight'][0,:].cuda()
            break_latent[0]=torch.load(path_tool_latent)['latent_codes']['weight']['weight'][0,:].cuda()
        #latent[0]=torch.load(os.path.join('/home/michael/Projects/DeepMend_changes3_new2/deepmend','experiments','mugs','ModelInference','{}_lat.pth'.format(0)))['latent_codes']['weight']['weight'][0,:].cuda()
        #break_latent[0]=torch.load(os.path.join('/home/michael/Projects/DeepMend_changes3_new2/deepmend','experiments','mugs','ModelInference','{}_tool_lat.pth'.format(0)))['latent_codes']['weight']['weight'][0,:].cuda()          
        #print(latent.size(),break_latent.size())
    else:
    ...
    for e in range(num_iterations):
        ...
        adjust_learning_rate_new(optimizer, lr_warmup,warmup_length,lr_end,e, num_iterations)
        ...
        if e % 10 == 0:
            tb_logger.add_scalar("loss", loss.item(), e)
            tb_logger.add_scalar("data_loss", data_loss.item(), e)
            tb_logger.add_scalar("prox_loss", prox_loss.item(), e)
